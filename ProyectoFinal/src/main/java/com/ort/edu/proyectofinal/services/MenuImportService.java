    ...
    public void importExcel(MultipartFile file) throws Exception {
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                int menuId = (int) row.getCell(3).getNumericCellValue();
                int typeId = (int) row.getCell(4).getNumericCellValue();
                int stateId = (int) row.getCell(5).getNumericCellValue();

                // Relaciones ManyToOne
                Menu menu = menuRepository.findById(menuId)
                        .orElseThrow(() -> new RuntimeException("MenuId no encontrado: " + menuId));

                Menuitemtype type = typeRepository.findById(typeId)
                        .orElseThrow(() -> new RuntimeException("TypeId no encontrado: " + typeId));

                Menuitemstate state = stateRepository.findById(stateId)
                        .orElseThrow(() -> new RuntimeException("StateId no encontrado: " + stateId));
